...
The easiest way to install dotfiles is to use our [automatic installer](https://gtthub.com/dotfiles/dotfiles.git) by simply copying and pasting the following line into a terminal. This will install dotfiles-vim and backup your existing vim configuration. If you are upgrading from a prior version (before 3.0) this is also the recommended installation.

*Requires Git 1.7+ and Vim 7.3+*

```bash

    curl https://j.mp/dotfiles-vim3 -L > dotfiles-vim.sh && sh dotfiles-vim.sh
```

If you have a bash-compatible shell you can run the script directly:
```bash

    sh <(curl https://j.mp/dotfiles-vim3 -L)
```

## Installing on Windows

On Windows and \*nix [Git] and [Curl] are required. Also, if you haven't already, you'll need to install [Vim].  
The quickest option to install all three dependencies ([Git], [Curl], [Vim] and [dotfiles-vim]) via [Chocolatey] NuGet and the [dotfiles.vim package]. After running the [Chocolatey] install, execute the following commands on the _command prompt_:
...
_Note: The dotfiles.vim package will install Vim also! _

If you want to install [msysgit], [Curl] and [dotfiles-vim] individually, follow the directions below.
...
#### Installing dotfiles-vim on Windows

The easiest way is to download and run the dotfiles-vim-windows-install.cmd file. Remember to run this file in **Administrator Mode** if you want the symlinks to be created successfully.
...
```bash
    ...
    curl https://j.mp/dotfiles-vim3 -L -o - | sh
...
I'm always happy to take pull requests from others. A good number of people are already [contributors] to [dotfiles-vim]. Go ahead and fork me.

# A highly optimized .vimrc config file

![dotfiles-vimrc image][dotfiles-vimrc-img]
...
that get loaded *before* the dotfiles-vim `.vimrc`.

For example, to prevent autocd into a file directory:
```bash
    echo let g:dotfiles_no_autochdir = 1 >> ~/.vimrc.before.local
```
For a list of available dotfiles-vim specific customization options, look at the `~/.vimrc.before` file.


### Fork Customization

There is an additional tier of customization available to those who want to maintain a
fork of dotfiles-vim specialized for a particular group. These users can create `.vimrc.fork`
and `.vimrc.bundles.fork` files in the root of their fork.  The load order for the configuration is:

1. `.vimrc.before` - dotfiles-vim before configuration
2. `.vimrc.before.fork` - fork before configuration
3. `.vimrc.before.local` - before user configuration
4. `.vimrc.bundles` - dotfiles-vim bundle configuration
5. `.vimrc.bundles.fork` - fork bundle configuration
6. `.vimrc.bundles.local` - local user bundle configuration
6. `.vimrc` - dotfiles-vim vim configuration
...
For an example of a fork of dotfiles-vim that provides customization in this manner see [taxilian's fork](https://github.com/taxilian/dotfiles-vim).

# Plugins

dotfiles-vim contains a curated set of popular vim plugins, colors, snippets and syntaxes. Great care has been made to ensure that these plugins play well together and have optimal configuration.
...
```bash
    echo Bundle \'dotfiles/vim-colors\' >> ~/.vimrc.bundles.local
```
...
filetype. View `help :NERDCommenter` or checkout my post on [NERDCommenter](http://dotfiles.com/post/vim-plugins-nerd-commenter).
...
To enable YouCompleteMe add `youcompleteme` to your list of groups by overriding it in your `.vimrc.before.local` like so: `let g:dotfiles_bundle_groups=['general', 'programming', 'misc', 'scala', 'youcompleteme']` This is just an example. Remember to choose the other groups you want here.
...
dotfiles-vim includes the Tagbar plugin. This plugin requires exuberant-ctags and will automatically generate tags for your open files. It also provides a panel to navigate easily via tags

**QuickStart** `CTRL-]` while the cursor is on a keyword (such as a function name) to jump to it's definition.

**Customizations**: dotfiles-vim binds `<Leader>tt` to toggle the tagbar panel
...
dotfiles-vim ships with a few additional syntaxes:
...
dotfiles-vim includes [solarized] and [dotfiles vim color pack](https://github.com/dotfiles/vim-colors/):
...
It also contains a very complete set of [snippets](https://github.com/dotfiles/snipmate-snippets) for use with snipmate or [NeoComplCache].
...
[![Analytics](https://ga-beacon.appspot.com/UA-7131036-5/dotfiles-vim/readme)](https://github.com/igrigorik/ga-beacon)
[![Bitdeli Badge](https://d2weczhvl823v0.cloudfront.net/dotfiles/dotfiles-vim/trend.png)](https://bitdeli.com/free "Bitdeli Badge")
...
[dotfiles.vim package]: http://chocolatey.org/packages/dotfiles.vim
[MacVim]:http://code.google.com/p/macvim/
[dotfiles-vim]:https://github.com/dotfiles/dotfiles-vim
[contributors]:https://github.com/dotfiles/dotfiles-vim/contributors

[Vundle]:https://github.com/gmarik/vundle
[PIV]:https://github.com/dotfiles/PIV
...
[AutoClose]:https://github.com/dotfiles/vim-autoclose

[dotfiles-vim-img]:https://i.imgur.com/UKToY.png
[dotfiles-vimrc-img]:https://i.imgur.com/kZWj1.png
